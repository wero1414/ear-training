...
export function showJam() {
  ...
  const h =
    ...
    '<button class="play" id="jamBtn">Play</button>' +
    '<select id="jamPreset">' +
    JAMP.map(
      (x, i) => '<option value="' + i + '"' + (i === S.jamPreset ? ' selected' : '') + '>' + x.n + '</option>',
    ).join('') +
    '</select>' +
    '<select id="jamKey">' +
    ...
    '<span class="hint">tempo</span><input type="range" id="jamBpm" min="50" max="180" step="1" value="' +
    S.jamBpm +
    ...
    (S.jamBass ? ' checked' : '') +
    '> bass</label>' +
    '<button class="mini" id="jamMidi">Export MIDI</button></div>' +
    '<div class="bars" id="jamBars"></div>' +
    '<div class="now" id="jamNow">Press play. Chord tones light up in solid teal, the rest of the scale in outline.</div>' +
    '<div id="jamKb"></div></div>';
  el('view').innerHTML = h;
  jam.bars = jamBars();
  el('jamBars').innerHTML = jam.bars
    .map(b => '<div class="bar">' + nn((S.jamKey + b.s) % 12) + CH[b.q].s + '<em>1 bar</em></div>')
    .join('');
  ...
  el('jamBpm').oninput = e => {
    ...
    el('bpmVal').textContent = S.jamBpm + ' bpm';
    sv();
  };
...
function exportMidi() {
  ...
  a.download =
    JAMP[S.jamPreset].n.replace(/[^a-z0-9]+/gi, '-').toLowerCase() + '-' + nn(S.jamKey).replace('#', 's') + '.mid';
  a.click();
